		...
		for (TSLPointer tSLPointer: iTSLObjectLotlDownload.getSchemeInformation().getPointersToOtherTSL()) {
			...
			if (!tSLPointer.getMimeType().contains("application/pdf")) {
				try {
					...
					String messageError = Language.getFormatResWebGeneral(IWebGeneralMessages.TASK_FIND_NEW_TSL_REV_LOG_026, new Object[ ] { UtilsCountryLanguage.getFirstLocaleCountryNameOfCountryCode(schemeTerritory) });
					LOGGER.error(messageError, e);
				}
			}
		}
