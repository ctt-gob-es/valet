	...
	private void validateInputsUpdate(ConfTslRegDTO confTslRegDTO, JSONObject json) {
		if(confTslRegDTO.getTslRegEnabled()) {
			if(confTslRegDTO.getIdModeReg() == NumberConstants.NUM_NEG_1) {
				String msgError = Language.getResWebGeneral(IWebGeneralMessages.LOG_CRT001);
				LOGGER.error(msgError);
				json.put(FIELD_SELECT_LIST_MODE_REG + "_span", msgError);
			}
			if(confTslRegDTO.getIdTypeFilterReg() == NumberConstants.NUM_NEG_1) {
				String msgError = Language.getResWebGeneral(IWebGeneralMessages.LOG_CRT002);
				LOGGER.error(msgError);
				json.put(FIELD_SELECT_LIST_TYPE_FILTER_REG + "_span", msgError);
			}
		}
	}
