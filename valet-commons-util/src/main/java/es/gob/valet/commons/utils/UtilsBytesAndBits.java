...
/**
 * <b>File:</b><p>es.gob.valet.commons.utils.UtilsBytesAndBits.java.</p>
...
public final class UtilsBytesAndBits {
	...
	public static byte[ ] bitToByteArray(BitSet bits) {
		...
		if (bits != null) {
			bytes = new byte[(bits.length() + NumberConstants.NUM7) / NumberConstants.NUM8];
			for (int i = 0; i < bits.length(); i++) {
				if (bits.get(i)) {
					bytes[bytes.length - i / NumberConstants.NUM8 - 1] |= 1 << i % NumberConstants.NUM8;
				}
			}
		}
	...
	public static BitSet byteToBits(byte b) {
		byte aux = b;
		BitSet bits = new BitSet(NumberConstants.NUM8);
		for (int i = 0; i < NumberConstants.NUM8; i++) {
			bits.set(i, (aux & 1) == 1);
			aux >>= 1;
		}
		return bits;
	...
	public static BitSet byteArrayToBits(byte[ ] byteArray) {
		...
		BitSet bits = null;
		if (byteArray != null) {
			bits = new BitSet(byteArray.length * NumberConstants.NUM8);
			for (int i = 0; i < byteArray.length; i++) {

				byte b = byteArray[i];
				int bitBasePosition = i * NumberConstants.NUM8;
				for (int j = 0; j < NumberConstants.NUM8; j++) {
