...
public class ImporTslService implements IImporTslService {
	...
	private void resetProcess() {
		LOGGER.info(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP001));
		isRunning = false;
		isError = false;
		currentStep.set(1);
	...
	public void enableTslRelatedTask() throws ImporTslsException {
		LOGGER.info(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP002));
		try {
			...
			LOGGER.error(e);
			throw new ImporTslsException(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP003));
		}
	}
	...
	private void importMappingByService() {
		LOGGER.info(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP004));
	...
	private void importDataTsl() throws ImporTslsException {
		LOGGER.info(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP006));
	...
	private void saveNewTsl(TslDataDTO tslDataDTO, String countryRegionCode, TslCountryRegion tslCountryRegion) throws ImporTslsException {
		try {
			...
			LOGGER.error(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP007), e);
			throw new ImporTslsException();
		} catch (TSLCacheException e) {
			LOGGER.error(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP008), e);
			throw new ImporTslsException();
	...
	private void updateTsls(TslDataDTO tslDataDTO, TslCountryRegion tslCountryRegion) throws ImporTslsException {
		try {
			...
			LOGGER.error(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP009), e);
			throw new ImporTslsException();
		} catch (ParseException e) {
			LOGGER.error(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP010), e);
			throw new ImporTslsException();
		} catch (TSLCacheException e) {
			LOGGER.error(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP011), e);
			throw new ImporTslsException();
		} catch (CertificateEncodingException e) {
			LOGGER.error(Language.getResCommonsUtilGeneral(CommonsUtilGeneralMessages.UTILS_CERTIFICATE_002), e);
			throw new ImporTslsException();
	...
	private void readZipFile() throws ImportException {
	    ...
	    try (ZipInputStream zipStream = new ZipInputStream(contentFile)) {
	        ...
	        LOGGER.error(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP012), e);
	        throw new ImportException();
	    }

	    LOGGER.info(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP013));
		...
		if (!entry.isDirectory()) {
			...
			try {
				...
				if (entry.getName().startsWith("tslData/")) {
					...
					LOGGER.info(Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP014, new Object[ ] { entry.getName() }));
				...
				LOGGER.error(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP015), e);
				throw new ImportException();
			} finally {
				// Cerramos recursos
	...
	public void disableTslRelatedTask() throws ImporTslsException {
		LOGGER.info(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP016));
		...
		try {
			if (tasksScheduler.checkIfExistsTask(taskTslSyncro)) {
				...
				LOGGER.warn(Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP017, new Object[ ] { taskTslSyncro }));
			}
			...
			if (tasksScheduler.checkIfExistsTask(taskExternalAccess)) {
				...
				LOGGER.warn(Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP017, new Object[ ] { taskExternalAccess }));
			}
			getAllProgress()[NumberConstants.NUM0] = NumberConstants.NUM100;
		}  catch (ValetSchedulerException e) {
			LOGGER.error(e);
			throw new ImporTslsException(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP018));
		}
	}
	/**
	 * 
	...
	public String getSbSummaryImport() {
		...
		if(this.overwrite) {
			...
			sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP020) + LINE_BREAK);
		}
		
		if(!lisTslDataSummary.isEmpty() && lisTslDataSummary.size() > NumberConstants.NUM0) {
			sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP021) + LINE_BREAK);
			
			for (TslDataSummary tslDataSummary: lisTslDataSummary) {
				sbSummaryImport.append(TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP022, new Object[ ] { tslDataSummary.getCountry() }) + LINE_BREAK);
				sbSummaryImport.append(TAB + TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP023, new Object[ ] { tslDataSummary.getNumberSequence() }) + LINE_BREAK);
		...
		if(!listMappingByTslSummary.isEmpty() && listMappingByTslSummary.size() > NumberConstants.NUM0) {
			sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP028) + LINE_BREAK);
			...
			for (Map.Entry<String, List<MappingByTslSummary>> entry : groupByCountry.entrySet()) {
			    ...
			    sbSummaryImport.append(TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP022, new Object[ ] { country }) + LINE_BREAK);
			    
			    for (MappingByTslSummary mappingByTslSummary: listMappingByTslSummary) {
			    	sbSummaryImport.append(TAB + TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP029, new Object[ ] { mappingByTslSummary.getIdentificator() }) + LINE_BREAK);
			    	sbSummaryImport.append(TAB + TAB + TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP030, new Object[ ] { mappingByTslSummary.getAssociationType() }) + LINE_BREAK);
					sbSummaryImport.append(TAB + TAB + TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP031, new Object[ ] { mappingByTslSummary.getValue() }) + LINE_BREAK);
			    }
			}
		}
		
		if(!listMappingByServDTO.isEmpty() && listMappingByServDTO.size() > NumberConstants.NUM0) {
			sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP032) + LINE_BREAK);
			...
			for (Map.Entry<String, List<MappingByServSummary>> entry : groupByCountry.entrySet()) {
				...
				for (Map.Entry<String, List<MappingByServSummary>> entry2 : groupByTspAndService.entrySet()) {
					sbSummaryImport.append(TAB + TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP033, new Object[ ] { entry2.getKey().split("&")[0] }) + LINE_BREAK);
					sbSummaryImport.append(TAB + TAB + TAB + Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP034, new Object[ ] { entry2.getKey().split("&")[1] }) + LINE_BREAK);
		...
		if(this.overwrite) {
			...
			if(message != null && message.length() > NumberConstants.NUM0) {
				sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP038) + LINE_BREAK);
				sbSummaryImport.append(message);
			}
		} else {
			if(listTslDataNotImpByNotOverrite.size()>NumberConstants.NUM0) {
				sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP039) + LINE_BREAK);
			...
			if(listTslDataNotImpByVersionMinor.size()>NumberConstants.NUM0) {
				sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP040) + LINE_BREAK);
			...
			if(listMappingByTslNotImpByNotOverrite.size()>NumberConstants.NUM0) {
				sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP041) + LINE_BREAK);
			...
			if(listMappingByServNotImpByNotOverrite.size()>NumberConstants.NUM0) {
				sbSummaryImport.append(Language.getResWebGeneral(WebGeneralMessages.LOG_IMP042) + LINE_BREAK);
	...
	public String getListTslDataNotImpByVersionMinor() {
		...
		if(listTslDataNotImpByVersionMinor != null && !listTslDataNotImpByVersionMinor.isEmpty()) {
			...
			message = Language.getFormatResWebGeneral(WebGeneralMessages.LOG_IMP043, new Object[ ] { countries });
		}
		return message;
	}
