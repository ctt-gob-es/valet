...
public final class CAdESTriPhaseSigner {
     ...
     * @throws SignatureException Cuando se produce cualquier error durante el proceso. */
    public static byte[] preSign(
        ...
        try {
            ...
            throw new SignatureException("Error obteniendo los atributos a firmar: " + e, e); //$NON-NLS-1$
        }
        ...
        try {
            ...
            throw new SignatureException("Error al codificar los datos ASN.1 a firmar finalmente", ex); //$NON-NLS-1$
        }
    ...
    public static byte[] postSign(final String signatureAlgorithm,
                                  ...
                                  final byte[] signedAttributes) throws SignatureException {
        ...
        final String digestAlgorithmName = SigUtils.getDigestAlgorithmName(signatureAlgorithm);
...
//        }
        ...
        try {
        	tbsCertificate = TBSCertificate.getInstance(
            		ASN1Primitive.fromByteArray(
    				((X509Certificate) signerCertificateChain[0]).getTBSCertificate()
				)
    		);
            ...
            throw new SignatureException("No se ha podido crear la estructura de certificados", e); //$NON-NLS-1$
        }

        final SignerIdentifier signerIdentifier = new SignerIdentifier(
           new IssuerAndSerialNumber(
    		   X500Name.getInstance(tbsCertificate.getIssuer()),
        ...
        try {
            ...
            throw new SignatureException("Error obteniendo el OID en ASN.1 del algoritmo de huella digital: " + e, e); //$NON-NLS-1$
        }
        ...
        try {
            ...
            throw new SignatureException("Error al codificar el algoritmo de cifrado: " + e, e); //$NON-NLS-1$
        }
        ...
        try {
            ...
            throw new SignatureException("Error en la inclusion de la recuperacion de los SignedAttibutes", e); //$NON-NLS-1$
        }
        ...
        if (content != null) {
            ...
            try {
                ...
                throw new SignatureException("Error en la escritura del contenido implicito en el ContentInfo", e); //$NON-NLS-1$
            }
            contentInfo = new ContentInfo(
        		new ASN1ObjectIdentifier(
    				PKCSObjectIdentifiers.data.getId()
				),
        		// En la nueva versión de Bouncycastle, esta clase no existe:
				// new BERConstructedOctetString(baos.toByteArray())
        		// Suponemos que la equivalente, es BEROctetString ya que implementa a la interfaz ASN1Encodable
        		// que es lo que espera como segundo argumento el constructor new ASN1ObjectIdentifier
        ...
        for (final Certificate cert : signerCertificateChain) {
          ...
          // X509CertificateStructure está deprecado, usar org.bouncycastle.asn1.x509.Certificate  	
//                ce.add(
...
//        		);
        ...
        try {
			...
			throw new SignatureException("Error creando el ContentInfo de CAdES: " + e, e); //$NON-NLS-1$
		}
