...
public class ExportFileValidator {
	...
	private static ExportationCryptoFiles extractSignatureFiles(final byte[] exportFileData)
		...
		if (fileHash == null) {
			throw new SecurityException("El fichero esperado files_hash_v_"+ValetVersionDTO.VersionEnum.V1.getVersion()+".properties no se ha encontrado dentro de META-INF"); //$NON-NLS-1$
		}
		if (signature == null) {
