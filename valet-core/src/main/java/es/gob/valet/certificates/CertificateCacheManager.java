...
/** 
 * <p>Class that manages the certificates stored in the cache, obtained from the TSLs and those registered in the CA trust store.</p>
 * <b>Project:</b><p>Platform for detection and validation of certificates recognized in European TSL.</p>
 * @version 1.0, 11/04/2022.
 */
public final class CertificateCacheManager {
	...
	/**
	 * Attribute that represents the list of certificates that have been registered in the CA trust store.
	 */
	...
	/**
	 * Method that caches the different certificates stored in the CA trust store.
	 */
	public void loadListCertificateCA(){
		// obtenemos el almacén de claves de la caché
				...
				try {
					...
					List<X509Certificate> listX509CerCA = keystoreFacade.getAllX509Certificates();

					if (listX509CerCA != null && !listX509CerCA.isEmpty()) {
						listCertificateCA.addAll(listX509CerCA);
					}

				} catch (KeystoreCacheException | CryptographyException  e) {
	 ...
	 * @param listCertificateCA The value for the attribute {@link #listCertificateCA}.
	 */
