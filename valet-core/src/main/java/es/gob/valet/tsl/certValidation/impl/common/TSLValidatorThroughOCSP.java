...
public class TSLValidatorThroughOCSP implements ITSLValidatorThroughSomeMethod {
	...
	private boolean evaluateIfOCSPServiceIsReliable(Date validationDate, TSLValidatorResult validationResult, TrustServiceProvider tsp, ATSLValidator tslValidator, BasicOCSPResp basicOcspResponse, X509CertificateHolder[ ] basicOcspResponseSignerCerts) {
		...
		LOGGER.info(Language.getResCoreTsl(ICoreTslMessages.LOGMTSL427));
		...
		// respuesta ha sido emitido por algÃºn certificado registrado en el almacÃ©n de confianza CA (TrustStoreCA).
		if(!result) {
			result = issuedBySomeCertRegisteredInTrustStoreCA(signerCert);
			
			// 3. Si el emisor no se encuentra en el almacÃ©n de confianza CA, se comprueba si estÃ¡ registrado y 
	...
	*/
	private boolean checksRegisteredMarkedAsTrustedInTrustStoreOCSP(Certificate signerCert, ATSLValidator tslValidator) {
		...
		LOGGER.info(Language.getFormatResCoreTsl(ICoreTslMessages.LOGMTSL439, new Object[ ] { result }));
		return result;
	}
	...
	private boolean issuedBySomeCertRegisteredInTrustStoreCA(Certificate signerCert) {
		...
		if (signerCert != null) {			
			try {
		        ...
		        for (Map.Entry<String, X509Certificate> entry : mapAliasX509Certificate.entrySet()) {
		            X509Certificate certKeystoreOCSPX509 = entry.getValue();
		            // Si el firmante ha sido emitido por algÃºn certificado registrado en el almacÃ©n de confianza CA, lo consideramos como confiable.
					if(UtilsCertificate.isIssuer(certKeystoreOCSPX509, signerCertX509)) {
						LOGGER.info(Language.getResCoreTsl(ICoreTslMessages.LOGMTSL431));
						result = true;
						break; // Rompo el bucle mÃ¡s cercano ya que hemos encontrado un certificado cuyo firmante ha sido emitido.
					}
		        }
			} catch (CommonUtilsException e) {
				LOGGER.error(Language.getResRestGeneral(IRestGeneralMessages.REST_LOG012));
		...
		LOGGER.info(Language.getFormatResCoreTsl(ICoreTslMessages.LOGMTSL433, new Object[ ] { result }));
		return result;
	}
	...
	private boolean checkOCSPResponseIssuedBySomeDigitalIdentity(Date validationDate,
		...
		LOGGER.info(Language.getFormatResCoreTsl(ICoreTslMessages.LOGMTSL430, new Object[ ] { result }));
		return result;
	...
	private Certificate findSigningCertificate(BasicOCSPResp basicOcspResponse, X509CertificateHolder[ ] basicOcspResponseSignerCerts) {
		...
		if(null != signerCert) {
			LOGGER.info(Language.getFormatResCoreTsl(ICoreTslMessages.LOGMTSL428, new Object[ ] { signerCert.getSerialNumber() }));
		} else {
			LOGGER.warn(Language.getResCoreTsl(ICoreTslMessages.LOGMTSL429));
		}
	...
	public void searchRevocationValueCompatible(X509Certificate cert, BasicOCSPResp basicOcspResponse, X509CRL crl,
		...
		if (dip.isThereSomeDigitalIdentity() && (checkOCSPResponseIssuedBySomeDigitalIdentity(validationDate, validationResult, null, null, signerCert)
