...
public class HandshakeValidationMdm implements X509TrustManager {
	...
	public void checkServerTrusted(X509Certificate[] chain, String authType) throws CertificateException {
		LOGGER.debug(Language.getResCoreGeneral(CoreGeneralMessages.HVMDM_001));
			...
			for(int i=0;i<chain.length;i++) {
    			...
    			if(chain[i]!= null && x509Certificate!=null) {
    				...
    				if(UtilsCertificate.isIssuer(chain[i],x509Certificate ) || UtilsCertificate.equals(chain[i],x509Certificate)) {
    					accept = true;
        				LOGGER.info(Language.getResCoreGeneral(CoreGeneralMessages.HVMDM_002));
            	        break; 
    				}
    			}
    		}
    	} catch (Exception e) {
			throw new CertificateException(Language.getResCoreGeneral(CoreGeneralMessages.HVMDM_003));
		}
		
		if(!accept) {
			throw new CertificateException(Language.getResCoreGeneral(CoreGeneralMessages.HVMDM_004));
		}
