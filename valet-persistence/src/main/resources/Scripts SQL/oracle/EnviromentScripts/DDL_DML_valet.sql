...
COMMENT ON TABLE "TSL_SERVICE" IS 'Tabla que almacena toda la informaci贸n relativa a servicios TSL.';
COMMENT ON COLUMN "TSL_SERVICE"."ID_TSL_SERVICE" IS 'Clave primaria generada por secuencia.';
COMMENT ON COLUMN "TSL_SERVICE"."COUNTRY" IS 'C贸digo del pa铆s.';
COMMENT ON COLUMN "TSL_SERVICE"."TSL_VERSION" IS 'Versi贸n de la TSL.';
...
COMMENT ON COLUMN "TSL_SERVICE"."CERTIFICATE" IS 'Certificado de ejemplo. NO DEBE OBTENERSE EN LA L脫GICA DE SERVICIO';
...
COMMENT ON TABLE "TSL_MAPPING" IS 'Tabla que almacena toda la informaci贸n relativa a servicios TSL y campos l贸gicos.';
...
COMMENT ON COLUMN "TSL_MAPPING"."LOGICAL_FIELD_ID" IS 'Nombre del campo l贸gico. ej: nombreResponsable.';
COMMENT ON COLUMN "TSL_MAPPING"."LOGICAL_FIELD_VALUE" IS 'Valor del campo l贸gico.';
...
COMMENT ON COLUMN "EXTERNAL_ACCESS"."ID_URL" IS 'Identificador de la url de conexi贸n.';
COMMENT ON COLUMN "EXTERNAL_ACCESS"."URL" IS 'URL de conexi贸n.';
COMMENT ON COLUMN "EXTERNAL_ACCESS"."ORIGIN_URL" IS 'Valor que representa el origen de recuperaci贸n de las URLs.';
COMMENT ON COLUMN "EXTERNAL_ACCESS"."STATE_CONN" IS 'Valor que indica si la conexi贸n es correcta (Y) o no (N).';
COMMENT ON COLUMN "EXTERNAL_ACCESS"."LAST_CONN" IS 'Valor que indica la fecha de la 煤ltima conexi贸n.';
...
COMMENT ON COLUMN "SYSTEM_CERTIFICATE"."COUNTRY" IS 'Pa铆s del certificado.';

ALTER TABLE "SYSTEM_CERTIFICATE" ADD "COUNTRY" VARCHAR2(20);
COMMENT ON COLUMN "SYSTEM_CERTIFICATE"."COUNTRY" IS 'Pa铆s del certificado.';

ALTER TABLE EXTERNAL_ACCESS ADD ID_COUNTRY_REGION NUMBER(19,0) NOT NULL;
COMMENT ON COLUMN "EXTERNAL_ACCESS"."ID_COUNTRY_REGION" IS 'Clave foranea a la tabla de regiones de pa铆ses.';
...
COMMENT ON COLUMN "SYSTEM_CERTIFICATE"."VALIDATION_CERT" IS 'Valor que indica si el certificado es v谩lido (Y) o no (N).';

-- Table CONF_SERVER_MAIL
ALTER TABLE "CONF_SERVER_MAIL" ADD "CERTIFICATE_FILE" BLOB NULL;
COMMENT ON COLUMN "CONF_SERVER_MAIL"."CERTIFICATE_FILE" IS 'Columna que almacenar谩 el certificado que se usar谩 para enviar correos con cifrado TLS.';
ALTER TABLE "CONF_SERVER_MAIL" ADD "ORIGINAL_NAME_FILE" VARCHAR2(100) NULL;
COMMENT ON COLUMN "CONF_SERVER_MAIL"."ORIGINAL_NAME_FILE" IS 'Columna que almacenar谩 el nombre original del fichero que contiene el certificado.';
...
COMMENT ON TABLE "VALET_SERVER" IS 'Tabla que almacena la configuraci髇 de los servidores valET.';
COMMENT ON COLUMN "VALET_SERVER"."ID_VALET_SERVER" IS 'Identificador 鷑ico del servidor.';
COMMENT ON COLUMN "VALET_SERVER"."SERVER_NAME" IS 'Nombre descriptivo del servidor.';
COMMENT ON COLUMN "VALET_SERVER"."SERVER_URL" IS 'URL donde est醤 desplegados los servicios de valET.';
COMMENT ON COLUMN "VALET_SERVER"."APPLICATION" IS 'Identificador de la aplicaci髇 asociada.';
COMMENT ON COLUMN "VALET_SERVER"."VERSION" IS 'Versi髇 de valET: 1 si es v1.1, 2 si es v1.2 o superior.';
COMMENT ON COLUMN "VALET_SERVER"."CONNECTION_TIMEOUT" IS 'Tiempo de espera para la conexi髇.';

-- ######################################################## 2. CREACI脫N DE SEQUENCIAS ########################################################
ALTER SESSION SET CURRENT_SCHEMA="VALET_CONFIGOWNER";
...
-- Secuencia necesaria para la clave de la tabla VALET_SERVER.
CREATE SEQUENCE SQ_VALET_SERVER
  START WITH 1
  MAXVALUE 999999999999999999999999999
  MINVALUE 1
  NOCYCLE
...
-- ######################################################## 3. CREACI脫N DE PERMISOS DE USUARIO ########################################################
...
-- ######################################################## 4. CREACI脫N DE SINONIMOS PARA TABLAS Y SEQUENCIAS ########################################################
...
--Se a帽ade el campo NIF a la tabla USER_VALET
ALTER TABLE USER_VALET
ADD NIF VARCHAR2(150) DEFAULT '12345678Z' NOT NULL;

-- Eliminar restricci贸n columna LOGIN
ALTER TABLE "USER_VALET" 
...
-- Agregar la restricci贸n 煤nica a la columna NIF
ALTER TABLE "USER_VALET"
